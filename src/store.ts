...
export const MIN_NODES_TO_WIN = 5;
...
export function describeAction(action: GameAction): string {
  ...
  switch (action.type) {
    ...
    case 'mark_clear':
      return `cleared the gap on ${label}`;
    case 'annotate':
...
interface GameStore extends GameState {
  ...
  markClear: (nodeId: string, player: PlayerType) => MoveOutcome;
...
function blocked(state: GameState): string | null {
  ...
  if (state.gamePhase === 'ended' || state.gameStatus !== 'playing') return 'Game is over';
  return null;
}
...
export const useGameStore = create<GameStore>((set, get) => {
  ...
  /** State changes shared by every move that costs a turn. */
  ...
  return {
    ...
    plant: (rawLabel, player, content = {}) => {
      ...
      if (state.nodes.length > 0) return fail('Root already exists');
    ...
    branch: (parentId, rawLabel, kind, player, content = {}) => {
      ...
      spend(player, action, [...state.nodes, child]);
      return ok(`Added ${capitalize(kind)} "${label}" under "${parent.label}"`, child.id);
    },
    ...
    markClear: (nodeId, player) => {
      ...
      const action = record('mark_clear', player, true, { nodeId, label: node.label, kind: node.kind });
      spend(
        player,
        action,
        state.nodes.map(n => {
          if (n.id !== nodeId) return n;
          const cleared: TreeNode = { ...n, isGap: false };
          delete cleared.gapReason;
          return cleared;
        }),
      );
      return ok(`Cleared the gap on "${node.label}"`);
    },
    ...
    undoLastMove: () => {
      ...
      set({
        ...
        currentPlayer: 'human',
        movesRemaining: state.movesRemaining + (last.costsMove ? 1 : 0),
      });
    ...
    checkWinCondition: () => {
      ...
      const hasGaps = state.nodes.some(n => n.isGap);
      const won = !hasGaps && state.nodes.length >= MIN_NODES_TO_WIN;
      set({ gameStatus: won ? 'won' : 'lost', gamePhase: 'ended' });
    },
  };
});
